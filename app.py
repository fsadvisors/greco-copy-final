...
2. Adjust **Threshold Settings** if needed.  
...
def map_headers(df: pd.DataFrame) -> dict:
    """
    Returns a mapping raw_col → one STANDARD_HEADERS,
    using header-name similarity + content heuristics.
    """
    ...
    for col in raw_cols:
        ...
        metrics[col] = {'unique': uniq, 'numeric': num, 'date': date, 'gstin': gstin, 'alnum': alnum}

    # assignment order: (std, header_weight, content_key, content_weight)
    order = [
        ("GSTIN",        0.3, 'gstin',   0.7),
    ...
    for std, wh, ck, wc in order:
        ...
        if best_col is None:
            raise KeyError(f"No column found for {std}")
...
def read_with_header_detection(uploaded_file):
    ...
    candidates = ["supplier","party","vendor","invoice","voucher","bill","date","gstin","value","taxable","cgst","sgst","igst","cess"]
...
def clean_and_standardize(raw: pd.DataFrame, suffix: str) -> pd.DataFrame:
    ...
    # pick one raw col per standard
    std_to_raw = {}
    for raw_col, std in mapping.items():
        std_to_raw.setdefault(std, raw_col)

    selected = [std_to_raw[s] for s in STANDARD_HEADERS]
    df_sel = df[selected].copy()
    df_sel.columns = STANDARD_HEADERS[:]  # exactly ten
    ...
    df_sel[f"InvoiceDate{suffix}"] = pd.to_datetime(
        df_sel[f"InvoiceDate{suffix}"], errors="coerce",
...
def make_remark_logic(row, g_sfx, b_sfx, amt_tol, date_tol):
    ...
    if pd.notna(bd) and pd.notna(gd):
        d = abs((bd-gd).days)
        if d>date_tol: mismatches.append("⚠️ Mismatch of InvoiceDate")
        elif d>0: trivial = True
